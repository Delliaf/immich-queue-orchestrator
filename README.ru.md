...
<!-- translation-source: README.md; source-sha256: 481a10c8c8fd3c106f7f856869b338eb1880926b4f1a2f8f1172fd3dc1e93ec0 -->

[English](README.md)

Внешний контроллер фоновых очередей Immich для домашних серверов с ограниченными CPU и RAM.

> Статус: ранний релиз `0.1.5`. Контракты проверены по Immich `v3.1.0`. До первого запуска на реальной библиотеке используйте `dryRun: true` и сделайте резервную копию.
...
Во встроенной конфигурации образа уже заданы URL сервиса Immich, интервалы загрузки, последовательная обработка и экономные настройки Node.js. Имя `ORCHESTRATOR_API_KEY` специально относится только к этому приложению, чтобы другой сервис, например Immich Power Tools, мог использовать собственный ключ Immich в том же `.env`. API key и необязательный пароль панели читаются прямо из того же `.env`, который уже использует Immich. Настоящий `.env` исключён из Git и Docker build context; в репозитории остаётся только пустой `.env.example`. Пароль панели — обычный пароль нашей панели, не API token и не ключ Immich. Если пароль не используется, панель показывает заметное предупреждение. Не публикуйте такой режим в интернет.

Затем выполните `docker compose up -d immich-queue-orchestrator`, откройте `http://<ip-сервера>:8005`, при необходимости введите пароль панели и один раз нажмите «Включить автопилот». В armed idle управляемые очереди уже стоят на паузе, а появление загрузки обнаруживается обычно за 10 секунд и гарантированно настраивается ниже 30 секунд. Armed state хранится в named volume и переживает перезапуски.

Стандартная строка `8005:8005` работает и через LAN-адрес, и через ZeroTier-адрес сервера. На другом устройстве `127.0.0.1` указывает на само это устройство, а не на сервер; открывайте `http://<zerotier-ip-сервера>:8005`. Чтобы публиковать панель только через ZeroTier, замените строку порта на `<zerotier-ip-сервера>:8005:8005`.
...
2. Скопируйте конфигурацию:
   ...
   printf '%s' 'ВАШ_API_КЛЮЧ_IMMICH' > secrets/immich_api_key.txt
...
4. Откройте панель через SSH tunnel или на самом сервере: `http://127.0.0.1:8005`. Введите пароль панели.
