...
<body>
    ...
    <main>
      ...
      <div id="queues" class="tab">
        ...
        <section>
          ...
          <div class="table-wrap"><table><thead><tr><th>#</th><th data-i18n="queue">Queue</th><th data-i18n="policy">Policy</th><th data-i18n="checkMissing">Check missing</th><th data-i18n="found">Found</th><th data-i18n="order">Order</th></tr></thead><tbody id="queue-settings"></tbody></table></div>
        </section>
      </div>
  ...
  <script>
    ...
    function renderStatus(){
      ...
      const inventories=run&&run.stages?run.stages.filter(stageItem=>stageItem.discoveryCompletedAt).map(stageItem=>t('inventoryLine',{queue:qLabel(stageItem.queue),count:stageItem.inventoryCount})):[]; byId('inventory-summary').innerHTML=inventories.length?inventories.map(line=>'<div>'+escapeHtml(line)+'</div>').join(''):t('noInventory');
    ...
    function renderQueueSettings(){
      ...
      byId('queue-settings').innerHTML=settings.queues.map((item,index)=>{const stage=stages.find(candidate=>candidate.queue===item.queue); return '<tr><td>'+(index+1)+'</td><td class="queue-name">'+escapeHtml(qLabel(item.queue))+(item.queue==='facialRecognition'?'<div class="help">'+escapeHtml(language==='ru'?'После обнаружения лиц':'After face detection')+'</div>':'')+'</td><td><select data-policy="'+item.queue+'"><option value="managed" '+(item.policy==='managed'?'selected':'')+'>'+t('managed')+'</option><option value="always-running" '+(item.policy==='always-running'?'selected':'')+'>'+t('alwaysRunning')+'</option><option value="ignored" '+(item.policy==='ignored'?'selected':'')+'>'+t('ignored')+'</option></select></td><td><input type="checkbox" data-missing="'+item.queue+'" '+(item.checkMissing?'checked':'')+'></td><td>'+(stage?stage.inventoryCount:'—')+'</td><td><button class="small" data-move="up" data-index="'+index+'">↑</button> <button class="small" data-move="down" data-index="'+index+'">↓</button></td></tr>';}).join('');
    ...
    function renderSettings(){ if(!settings)return; const a=settings.automation,l=settings.loadGuard; byId('scan-autopilot').checked=a.scanOnAutopilotStart;byId('scan-manual').checked=a.scanOnManualStart;setValue('periodic-hours',a.periodicDiscoveryIntervalMs==null?'':a.periodicDiscoveryIntervalMs/3600000);setValue('discovery-settle',a.discoverySettleMs/1000);setValue('discovery-timeout',a.discoveryTimeoutMs/60000);setValue('inventory-hold',a.inventoryHoldMs/1000);setValue('quiet-minutes',a.uploadQuietPeriodMs/60000);byId('adaptive-enabled').checked=a.adaptiveQuietEnabled;setValue('per-asset-seconds',a.adaptiveQuietPerAssetMs/1000);setValue('max-quiet-minutes',a.adaptiveQuietMaxMs/60000);setValue('queue-quiet',a.queueQuietMs/1000);setValue('active-poll',a.activePollMs/1000);setValue('guarded-poll',a.guardedPollMs/1000);setValue('standby-poll',a.standbyPollMs/1000);setValue('load-mode',l.mode);setValue('sample-seconds',l.sampleIntervalMs/1000);setValue('window-seconds',l.movingAverageWindowMs/1000);setValue('pause-above',l.pauseAbove==null?'':l.pauseAbove);setValue('pause-for',l.pauseForMs/1000);setValue('resume-below',l.resumeBelow==null?'':l.resumeBelow);setValue('resume-for',l.resumeForMs/1000);byId('runtime-json').textContent=JSON.stringify(settings,null,2);renderQueueSettings(); }
    function collectSettings(){ const periodic=byId('periodic-hours').value.trim(); const pauseAbove=byId('pause-above').value.trim(); const resumeBelow=byId('resume-below').value.trim(); return {...settings,automation:{scanOnAutopilotStart:byId('scan-autopilot').checked,scanOnManualStart:byId('scan-manual').checked,periodicDiscoveryIntervalMs:periodic===''?null:ms('periodic-hours',3600000),discoverySettleMs:ms('discovery-settle',1000),discoveryTimeoutMs:ms('discovery-timeout',60000),inventoryHoldMs:ms('inventory-hold',1000),uploadQuietPeriodMs:ms('quiet-minutes',60000),adaptiveQuietEnabled:byId('adaptive-enabled').checked,adaptiveQuietPerAssetMs:ms('per-asset-seconds',1000),adaptiveQuietMaxMs:ms('max-quiet-minutes',60000),queueQuietMs:ms('queue-quiet',1000),activePollMs:ms('active-poll',1000),guardedPollMs:ms('guarded-poll',1000),standbyPollMs:ms('standby-poll',1000)},loadGuard:{mode:byId('load-mode').value,sampleIntervalMs:ms('sample-seconds',1000),movingAverageWindowMs:ms('window-seconds',1000),pauseAbove:pauseAbove===''?null:Number(pauseAbove),pauseForMs:ms('pause-for',1000),resumeBelow:resumeBelow===''?null:Number(resumeBelow),resumeForMs:ms('resume-for',1000)},queues:settings.queues}; }
    ...
    document.querySelectorAll('.save-settings').forEach(button=>button.addEventListener('click',saveSettings)); document.querySelectorAll('#automation input,#load input,#load select').forEach(element=>element.addEventListener('change',markDirty));
