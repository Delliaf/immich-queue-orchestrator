...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    #log-output { min-height:280px; max-height:62vh; overflow:auto; background:#0b1119; border-radius:9px; padding:8px; }
    .log-entry { margin-bottom:7px; padding:10px; border:1px solid var(--line); border-left:4px solid #61758c; border-radius:8px; background:#101923; }
    .log-entry.debug { border-left-color:var(--blue); }
    ...
    .log-head { display:grid; grid-template-columns:auto minmax(0,1fr) auto; align-items:start; gap:9px; }
    .log-badge { min-width:52px; padding:2px 6px; border-radius:5px; text-align:center; color:#dce8f4; background:#26364a; font-size:10px; font-weight:750; letter-spacing:.4px; }
    .log-message { min-width:0; overflow-wrap:anywhere; font-weight:600; }
    .log-time { color:var(--muted); font-size:11px; white-space:nowrap; }
    .log-context { margin:8px 0 0 61px; }
    .log-context summary { color:var(--blue); cursor:pointer; font-size:12px; }
    .log-context pre { margin-top:7px; max-height:300px; white-space:pre-wrap; overflow-wrap:anywhere; }
    .log-empty { padding:28px 12px; color:var(--muted); text-align:center; }
    ...
    @media(max-width:520px) { .grid{grid-template-columns:1fr}.brand{font-size:15px}.header-controls button:not(#language){display:none}.log-toolbar{display:block}.log-toolbar .actions{margin-top:10px}.log-head{grid-template-columns:auto minmax(0,1fr)}.log-time{grid-column:2}.log-context{margin-left:0} }
  </style>
</head>
<body>
  <header>
  ...
  <div class="layout">
    <nav>
      ...
      <button data-tab-button="logs" data-i18n="logsTab">Logs</button>
    ...
    <main>
      ...
      <div id="logs" class="tab">
        <div class="toolbar"><h1 data-i18n="logsTab">Logs</h1><button class="primary save-settings" data-i18n="saveSettings">Save settings</button></div>
        <section>
          <h2 data-i18n="logSettings">Log settings</h2>
          <p class="detail" data-i18n="logHelp">Normal is suitable for permanent use. Detailed records decisions and state changes; maximum also records every poll, queue snapshot, and Immich API request.</p>
          <div class="form-grid">
            <div class="field"><label data-i18n="logLevel">Log level</label><select id="log-level"><option value="error" data-i18n="logErrors">Errors only</option><option value="warn" data-i18n="logWarnings">Warnings and errors</option><option value="info" data-i18n="logNormal">Normal</option><option value="debug" data-i18n="logDetailed">Detailed</option><option value="trace" data-i18n="logMaximum">Maximum diagnostics</option></select></div>
            <div class="field"><label data-i18n="retainedLogs">Entries retained in RAM</label><input id="log-retained" type="number" min="100" max="20000" step="100"></div>
          </div>
        </section>
        <section>
          <div class="toolbar log-toolbar"><h2 data-i18n="recentLogs">Recent logs</h2><div class="actions"><button id="refresh-logs" data-i18n="refreshLogs">Refresh</button><button id="download-logs" class="primary" data-i18n="downloadDiagnostics">Download diagnostic report</button><button id="clear-logs" class="danger" data-i18n="clearLogs">Clear</button></div></div>
          <div class="form-grid" style="margin-bottom:12px">
            <div class="field"><label data-i18n="displayLogLevel">Show entries from</label><select id="log-filter"><option value="trace" data-i18n="logAll">All captured entries</option><option value="debug" data-i18n="logDetailed">Detailed</option><option value="info" data-i18n="logNormal">Normal</option><option value="warn" data-i18n="logWarnings">Warnings and errors</option><option value="error" data-i18n="logErrors">Errors only</option></select></div>
            <div class="field"><label data-i18n="searchLogs">Search message or details</label><input id="log-search" type="search"></div>
          </div>
          <div id="logs-meta" class="detail">—</div>
          <div id="log-output"><div class="log-empty">—</div></div>
        </section>
      </div>
  ...
  <script>
    ...
    function renderSettings(){ if(!settings)return; const a=settings.automation,l=settings.loadGuard,g=settings.logging; byId('scan-autopilot').checked=a.scanOnAutopilotStart;byId('scan-manual').checked=a.scanOnManualStart;setValue('processing-priority',a.processingPriority);setValue('periodic-hours',a.periodicDiscoveryIntervalMs==null?'':a.periodicDiscoveryIntervalMs/3600000);setValue('discovery-settle',a.discoverySettleMs/1000);setValue('discovery-timeout',a.discoveryTimeoutMs/60000);setValue('inventory-hold',a.inventoryHoldMs/1000);byId('transient-enabled').checked=a.transientCounterStabilizationEnabled;setValue('transient-window',a.transientCounterWindowMs/1000);setValue('transient-maximum',a.transientCounterMaxMs/60000);setValue('transient-drop',a.transientCounterMinimumDropPercent);setValue('quiet-minutes',a.uploadQuietPeriodMs/60000);byId('adaptive-enabled').checked=a.adaptiveQuietEnabled;setValue('per-asset-seconds',a.adaptiveQuietPerAssetMs/1000);setValue('max-quiet-minutes',a.adaptiveQuietMaxMs/60000);setValue('queue-quiet',a.queueQuietMs/1000);setValue('active-poll',a.activePollMs/1000);setValue('guarded-poll',a.guardedPollMs/1000);setValue('standby-poll',a.standbyPollMs/1000);setValue('load-mode',l.mode);byId('monitor-idle').checked=l.monitorInIdle;setValue('sample-seconds',l.sampleIntervalMs/1000);setValue('window-seconds',l.movingAverageWindowMs/1000);setValue('pause-above',l.pauseAbove==null?'':l.pauseAbove);setValue('pause-for',l.pauseForMs/1000);setValue('resume-below',l.resumeBelow==null?'':l.resumeBelow);setValue('resume-for',l.resumeForMs/1000);setValue('log-level',g.level);setValue('log-retained',g.retainedEntries);byId('runtime-json').textContent=JSON.stringify(settings,null,2);renderQueueSettings(); }
    function collectSettings(){ const periodic=byId('periodic-hours').value.trim(); const pauseAbove=byId('pause-above').value.trim(); const resumeBelow=byId('resume-below').value.trim(); return {...settings,automation:{scanOnAutopilotStart:byId('scan-autopilot').checked,scanOnManualStart:byId('scan-manual').checked,processingPriority:byId('processing-priority').value,periodicDiscoveryIntervalMs:periodic===''?null:ms('periodic-hours',3600000),discoverySettleMs:ms('discovery-settle',1000),discoveryTimeoutMs:ms('discovery-timeout',60000),inventoryHoldMs:ms('inventory-hold',1000),transientCounterStabilizationEnabled:byId('transient-enabled').checked,transientCounterWindowMs:ms('transient-window',1000),transientCounterMaxMs:ms('transient-maximum',60000),transientCounterMinimumDropPercent:numberValue('transient-drop'),uploadQuietPeriodMs:ms('quiet-minutes',60000),adaptiveQuietEnabled:byId('adaptive-enabled').checked,adaptiveQuietPerAssetMs:ms('per-asset-seconds',1000),adaptiveQuietMaxMs:ms('max-quiet-minutes',60000),queueQuietMs:ms('queue-quiet',1000),activePollMs:ms('active-poll',1000),guardedPollMs:ms('guarded-poll',1000),standbyPollMs:ms('standby-poll',1000)},loadGuard:{mode:byId('load-mode').value,monitorInIdle:byId('monitor-idle').checked,sampleIntervalMs:ms('sample-seconds',1000),movingAverageWindowMs:ms('window-seconds',1000),pauseAbove:pauseAbove===''?null:Number(pauseAbove),pauseForMs:ms('pause-for',1000),resumeBelow:resumeBelow===''?null:Number(resumeBelow),resumeForMs:ms('resume-for',1000)},logging:{level:byId('log-level').value,retainedEntries:numberValue('log-retained')},queues:settings.queues}; }
    function renderLogEntries(result){const query=byId('log-search').value.trim().toLocaleLowerCase();const entries=result.entries.filter(entry=>!query||(entry.message+' '+JSON.stringify(entry.context||{})).toLocaleLowerCase().includes(query)).reverse();const output=byId('log-output');output.replaceChildren();if(!entries.length){const empty=document.createElement('div');empty.className='log-empty';empty.textContent=t('noLogs');output.appendChild(empty);}for(const entry of entries){const article=document.createElement('article');article.className='log-entry '+entry.level;const head=document.createElement('div');head.className='log-head';const badge=document.createElement('span');badge.className='log-badge';badge.textContent=entry.level.toUpperCase();const message=document.createElement('span');message.className='log-message';message.textContent=entry.message;const time=document.createElement('time');time.className='log-time';time.dateTime=entry.timestamp;time.title=entry.timestamp;time.textContent=new Date(entry.timestamp).toLocaleString(language==='ru'?'ru-RU':'en-GB');head.append(badge,message,time);article.appendChild(head);if(entry.context){const details=document.createElement('details');details.className='log-context';const summary=document.createElement('summary');summary.textContent=t('logDetails');const context=document.createElement('pre');context.textContent=JSON.stringify(entry.context,null,2);details.append(summary,context);article.appendChild(details);}output.appendChild(article);}byId('logs-meta').textContent=t('logSummary',{shown:entries.length,capacity:result.capacity,dropped:result.dropped});}
    async function refreshLogs(){ try{const limit=Math.min(settings?settings.logging.retainedEntries:1000,1000);lastLogSnapshot=await api('/api/logs?level='+encodeURIComponent(byId('log-filter').value)+'&limit='+limit);renderLogEntries(lastLogSnapshot);}catch(error){const output=byId('log-output');output.replaceChildren();const empty=document.createElement('div');empty.className='log-empty';empty.textContent=error.message;output.appendChild(empty);} }
    async function downloadDiagnostics(){ const response=await fetch('/api/logs/download',{headers:headers()});if(!response.ok){const body=await response.json().catch(()=>({}));throw new Error(body.message||'HTTP '+response.status);}const blob=await response.blob();const disposition=response.headers.get('content-disposition')||'';const filename=/filename="([^"]+)"/.exec(disposition)?.[1]||'immich-queue-orchestrator-diagnostics.json';const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download=filename;document.body.appendChild(link);link.click();link.remove();URL.revokeObjectURL(url); }
    ...
    async function refresh(){ if(authenticationFailed)return; try{const responses=await Promise.all([api('/api/status'),api('/api/config/effective'),api('/api/settings')]);currentStatus=responses[0];currentConfig=responses[1];if(!settingsDirty)settings=responses[2];byId('config').textContent=JSON.stringify(currentConfig,null,2);renderStatus();if(!settingsDirty)renderSettings();if(activeTab==='logs')await refreshLogs();}catch(error){if(error.message.includes('password')||error.message.includes('Unauthorized'))authenticationFailed=true;const banner=byId('banner');banner.className='banner error';banner.textContent=error.message+(authenticationFailed?t('providePassword'):'');} }
    ...
    document.querySelectorAll('[data-tab-button]').forEach(button=>button.addEventListener('click',()=>{activeTab=button.dataset.tabButton;document.querySelectorAll('[data-tab-button]').forEach(item=>item.classList.toggle('active',item===button));document.querySelectorAll('.tab').forEach(tab=>tab.classList.toggle('active',tab.id===activeTab));if(activeTab==='logs')refreshLogs();}));
    document.querySelectorAll('.save-settings').forEach(button=>button.addEventListener('click',saveSettings)); document.querySelectorAll('#automation input,#automation select,#load input,#load select,#log-level,#log-retained').forEach(element=>element.addEventListener('change',markDirty));
    byId('refresh-logs').addEventListener('click',refreshLogs);byId('log-filter').addEventListener('change',refreshLogs);byId('log-search').addEventListener('input',()=>{if(lastLogSnapshot)renderLogEntries(lastLogSnapshot);});byId('download-logs').addEventListener('click',()=>downloadDiagnostics().catch(error=>alert(error.message)));byId('clear-logs').addEventListener('click',async()=>{if(!confirm(t('clearLogsQuestion')))return;try{await api('/api/logs',{method:'DELETE'});await refreshLogs();}catch(error){alert(error.message);}});
